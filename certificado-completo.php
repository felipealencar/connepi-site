...
if(isset($_POST['email']) && isset($_POST['gerar'])){
  ...
  if($__user){
    ...
    if(count($erros) < 1){
      ...
      Certificamos que <b>$nome_avaliador</b> avaliou os seguintes artigos submetidos no
      XI CONNEPI – Congresso Norte Nordeste de Pesquisa e Inovação:
      <b style='font-size:18px;'>Maceió, 10 de novembro de 2016</b>");

      $count = 0;
      $mpdf->AddPage();
      $mpdf->WriteHTML("</div><div style='padding:200px 10px 0 30px;text-align:center;'>");
      foreach($__titles as $title){
        $mpdf->WriteHTML($title['setting_value'] . "<hr/>");
        $count++;
        if($count == 3){
          $mpdf->AddPage();
          $mpdf->WriteHTML("</div><div style='padding:200px 10px 0 30px;text-align:center;'>");
      ...
      $mpdf->Output("cert/gerados/avaliador-completo/$filename.pdf", "F");
    }
    else {
  ...
  <?php if(count($downloads) >= 1): ?>
    <div class="row" style="margin-top:5px" id="pdflist">
      <div class="list-group">
        <a href="cert/gerados/avaliador-completo/<?php echo $filename; ?>.pdf" target="_blank" class="btn btn-success">
